...
class RosbagSequentialDataset(Dataset):
    def __init__(self, csv_file, seq_len=4, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(idx, idx + self.seq_len):
            ...
            images = torch.cat(images, dim=0)  # multi-camera stack
            image_seq.append(images)

        numeric_seq = torch.stack(numeric_seq)       # (seq_len, numeric_dim)
        image_seq = torch.stack(image_seq)           # (seq_len, C, H, W)
        target = torch.tensor(
            self.data.iloc[idx + self.seq_len - 1][self.target_cols].astype(float).values,
            dtype=torch.float32
        )

        return numeric_seq, image_seq, target
